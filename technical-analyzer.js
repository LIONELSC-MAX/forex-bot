...
function analyzeTechnical(candles, pair, timeframe) {
  ...
  if (ma5 !== null && ma10 !== null) {
    if (ma5 > ma10) { bullScore++; signals.push('MA_BULL'); reasons.push('MA5 > MA10'); }
    else { bearScore++; signals.push('MA_BEAR'); reasons.push('MA5 < MA10'); }
  }

  // 2. EMA50 trend
  if (ema50 !== null) {
    if (lastClose > ema50) { bullScore++; signals.push('EMA50_BULL'); reasons.push('harga di atas EMA50'); }
    else { bearScore++; signals.push('EMA50_BEAR'); reasons.push('harga di bawah EMA50'); }
  }

  // 3. EMA200 trend jangka panjang (Golden/Death Cross)
  if (ema200 !== null) {
    if (lastClose > ema200) { bullScore += 1.5; signals.push('EMA200_BULL'); reasons.push('harga di atas EMA200 (uptrend)'); }
    else { bearScore += 1.5; signals.push('EMA200_BEAR'); reasons.push('harga di bawah EMA200 (downtrend)'); }
  }

  // 4. RSI
  let rsiNote = '';
  if (rsiValue !== null) {
    if (rsiValue < 30) { bullScore += 1.5; signals.push('RSI_BULL'); rsiNote = 'RSI oversold'; }
    else if (rsiValue > 70) { bearScore += 1.5; signals.push('RSI_BEAR'); rsiNote = 'RSI overbought'; }
    else if (rsiValue < 45) { bullScore += 0.5; rsiNote = 'RSI mendekati oversold'; }
    else if (rsiValue > 55) { bearScore += 0.5; rsiNote = 'RSI mendekati overbought'; }
  ...
  if (macdData) {
    if (macdData.histogram !== null) {
      if (macdData.histogram > 0 && macdData.macdLine > 0) { bullScore += 1.5; signals.push('MACD_BULL'); macdNote = 'MACD bullish'; }
      else if (macdData.histogram < 0 && macdData.macdLine < 0) { bearScore += 1.5; signals.push('MACD_BEAR'); macdNote = 'MACD bearish'; }
      else if (macdData.histogram > 0) { bullScore += 0.5; macdNote = 'MACD histogram positif'; }
      else { bearScore += 0.5; macdNote = 'MACD histogram negatif'; }
    }
  }
  ...
  let bbNote = '';
  if (bb) {
    if (lastClose <= bb.lower) { bullScore += 1.5; signals.push('BB_BULL'); bbNote = 'harga di lower BB (oversold)'; }
    else if (lastClose >= bb.upper) { bearScore += 1.5; signals.push('BB_BEAR'); bbNote = 'harga di upper BB (overbought)'; }
    else if (lastClose < bb.middle) { bullScore += 0.3; bbNote = 'harga di bawah mid BB'; }
    else { bearScore += 0.3; bbNote = 'harga di atas mid BB'; }
  }

  // 7. Stochastic
  let stochNote = '';
  if (stoch !== null) {
    if (stoch < 20) { bullScore += 1.5; signals.push('STOCH_BULL'); stochNote = 'Stochastic oversold'; }
    else if (stoch > 80) { bearScore += 1.5; signals.push('STOCH_BEAR'); stochNote = 'Stochastic overbought'; }
  ...
  const trend = ema50 && ema200
    ...
    : (ma5 && ma20 ? (ma5 > ma20 * 1.0005 ? 'UPTREND' : ma5 < ma20 * 0.9995 ? 'DOWNTREND' : 'SIDEWAYS') : 'SIDEWAYS');
  ...
  return {
    ...
    indicators: {
      ...
      macdSignal: macdData?.signalLine !== null ? macdData.signalLine?.toFixed(decimals) : null,
      macdHistogram: macdData?.histogram !== null ? macdData.histogram?.toFixed(decimals) : null,
