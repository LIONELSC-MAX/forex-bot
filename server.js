...
const SCAN_INTERVAL_MS =
  (parseInt(process.env.SCAN_INTERVAL_SECONDS) || 300) * 1000;
const WATCHED_PAIRS = (process.env.WATCHED_PAIRS || "EUR/USD,GBP/USD,USD/JPY")
  .split(",")
  .map((p) => p.trim());
...
async function fetchCandles(pair, timeframe) {
  const interval =
    timeframe === "1h" ? "1h" : timeframe === "1day" ? "1day" : "4h";
  ...
  if (data.status === "error" || !data.values) {
    throw new Error(
      `Twelve Data error untuk ${pair}: ${data.message || "unknown error"}`,
    );
  }
...
async function runScan() {
  console.log(
    `[${new Date().toISOString()}] Memulai scan ${WATCHED_PAIRS.length} pair...`,
  );
...
function startAutoScan() {
  ...
  scanTimer = setInterval(() => {
    runScan().catch((err) =>
      console.error("Scan terjadwal gagal:", err.message),
    );
  }, SCAN_INTERVAL_MS);
  console.log(
    `Auto-scan aktif, interval setiap ${SCAN_INTERVAL_MS / 1000} detik`,
  );
}
...
// === Endpoint: ambil data candle untuk chart ===
